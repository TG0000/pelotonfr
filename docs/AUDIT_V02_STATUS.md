...
- 13 tests unitaires et 11 tests d’intégration réussis sur la base dédiée. Aucun compte ou jeton de production copié.
- Build local sous Node 22 réussi ; schéma d’authentification inspecté ; validation CI du nouveau commit à attendre.
...
| SEC-08 | P1 | Les écritures publiques n’ont pas de budget d’abus visible dans l’application | Budgets atomiques par compte/visiteur et garde globale contact ; protections d’origine sur les mutations API. Test concurrent du compteur. Contact reste sur budget global ; WAF fournisseur non configuré. |
| SEC-09 | P2 | La validation des paramètres n’est pas homogène | UUID, corps JSON bornés, origine, coordonnées, filtres et retour local validés sur les mutations principales. Tests null/array/malformed/oversized ; revue exhaustive des paramètres de pages encore nécessaire. |
| SEC-10 | P2 | Les réponses n’appliquent pas plusieurs protections navigateur usuelles | En-têtes CSP défensifs, anti-frame, nosniff, referrer et noindex preview. Contrôle HTTP de la nouvelle preview à consigner. |
...
| UX-03 | P1 | Les textes secondaires et annulations manquent de contraste | Palette renforcée et opacité des textes de lieu supprimée après axe. Recette clair/sombre finale à compléter. |
...
| UX-07 | P2 | La connexion perd l’intention d’ajouter une course | Course conservée 15 minutes dans sessionStorage, retour vers la fiche et ajout après connexion ; ouverture de dialogue et mémorisation vérifiées. |
...
| PERF-06 | P2 | Les lectures répétées et limites de listes demandent une optimisation mesurée | React.cache sur lectures partagées/session/course ; limite carte explicitée à 2 000. Profil de requêtes à volume représentatif encore à produire. |
...
| OPS-01 | P1 | La recette de préversion échouait malgré un déploiement READY | Base isolée et secrets propres à la branche ; 71 migrations appliquées, données fictives et connexion synthétique testées localement. Recette du nouveau déploiement à terminer. |
...
| NEW-03 | P2 | Une erreur du chargement Google reste mémorisée pour tous les essais | Promesse Google rejetée réinitialisée, script retiré, timeout de chargement complet, nettoyage des écouteurs. Réessai réutilise sa réservation ; scénario fournisseur bloqué/retabli à automatiser. |
| NEW-04 | P1 | Une panne de métadonnées peut devenir une couverture nulle pendant 180 jours | Réponse inconnue n’écrit aucune couverture ; hash du tracé lie cache et géométrie, couverture périmée ignorée. Test fournisseur simulé restant à ajouter. |
...
1. Terminer les scénarios navigateur et la recette de la nouvelle preview, puis les mesures à volume représentatif.
