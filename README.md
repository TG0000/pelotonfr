...
- Street View intégré exige `GOOGLE_MAPS_BROWSER_KEY`, `STREETVIEW_DAILY_CAP` et `STREETVIEW_MONTHLY_CAP`. Les deux plafonds se comptent en **panoramas**, pas en ouvertures : Google facture chaque panorama chargé, et la visite en charge un tous les cent cinquante mètres, soit une quarantaine sur une boucle de cinq kilomètres. Une ouverture réserve d’avance ce que son tour peut coûter. Ces plafonds ne sont pas des plafonds de facture : restrictions API/domaine, quota posé dans la console Google et suivi de facturation restent nécessaires.
