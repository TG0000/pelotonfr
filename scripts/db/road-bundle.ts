...
async function main() {
  ...
  const views = await sql(`SELECT picture_id, along_m, taken_on::text, url, producer, reading, bearing, orientation, encode(crop, 'base64') AS crop64 FROM road_views WHERE race_id = $1::uuid ORDER BY along_m`, [id]);
