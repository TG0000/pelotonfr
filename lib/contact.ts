...
export async function createContact(category: string, message: string, visitor: string) {
  ...
  if (!(await consumeLimit(`contact:create:${visitor}`, 5, 3600)) || !(await consumeLimit("contact:create:global", 20, 60))) {
...
export async function lookupContact(id: string, code: string, visitor: string) {
  if (!isUuid(id) || !/^[\w-]{32}$/.test(code)) return { error: "Référence ou code incorrect." };
  if (!(await consumeLimit(`contact:read:${visitor}`, 30, 60)) || !(await consumeLimit("contact:read:global", 120, 60))) return { error: "Réessaie dans une minute." };
  const hash = createHash("sha256").update(code).digest("hex");
