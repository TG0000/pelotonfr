...
export default async function HomePage() {
  ...
  return <div className="mx-auto w-full max-w-7xl px-5 sm:px-8">
    ...
    <section id="rendez-vous" className="scroll-mt-24 py-7">
      ...
      {unavailable ? <div role="status" className="rounded-xl border p-6">Les prochaines courses ne peuvent pas être chargées pour le moment. Réessaie dans quelques instants.</div> : upcomingRaces.length ? <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{upcomingRaces.map(race=><RaceCard key={race.id} race={race} />)}</div> : <p className="rounded-xl border p-6 text-muted-foreground">Aucune prochaine course n’est encore référencée. Reviens après la prochaine mise à jour du calendrier.</p>}
    </section>
